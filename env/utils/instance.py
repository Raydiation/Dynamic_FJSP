...
class JSP_Instance:
    ...
    def update_time(self):
        ...
        for m in self.machines:
            if m.breakdown_time == self.current_time and m.avai_time() > self.current_time:
                ...
                if self.use_log:
                    ...
                    # print('log : {} _ {} ~ {}'.format(m.machine_id, m.breakdown_time, m.repair_time))
                m.next_breakdown_event()
                self.register_time(m.breakdown_time)
                self.register_time(m.breakdown_time + m.repair_time)

            elif m.breakdown_time + m.repair_time <= self.current_time:
                if self.use_log:
                    self.log(-1, 0, m.machine_id, m.breakdown_time, m.repair_time)
                    # print('log : {} _ {} ~ {}'.format(m.machine_id, m.breakdown_time, m.repair_time))
                m.next_breakdown_event()
